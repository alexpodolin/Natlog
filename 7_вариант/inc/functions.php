  ...
  for ($i = $date; $i <= $date; ($i+=86400))
    {
      ...
      //min_datetime_log из таблицы
      //echo $min_datetime_log = "{$min_d_year}-{$d_month}-{$d_day} {$h_hour}:{$t_minute}:00"; 
      $min_datetime_log = date('Y-m-d H:i:s', strtotime("{$min_d_year}-{$d_month}-{$d_day} {$h_hour}:{$t_minute}:00")-(3600*3));       
      //min_datetime_log из таблицы + 5 минут
      ...
      //echo $min_datetime_log_ts = date('Y-m-d H:i:s', strtotime($min_datetime_log)-(3600*3));      
      ...
      if($t_exist[count])
      {
        ...
        $sql_get_data = $db_conn->query("SELECT * FROM logs_table_{$t_name} WHERE true {$where} AND min_datetime_log>='{$min_datetime_log}' AND min_datetime_log<='{$min_datetime_log_plus}' ORDER BY min_datetime_log DESC");
   
//echo "SELECT * FROM logs_table_{$t_name} WHERE true {$where} AND min_datetime_log>='{$min_datetime_log}' AND min_datetime_log<='{$min_datetime_log_plus}' ORDER BY min_datetime_log DESC LIMIT 100";
        ...
        //Вставка данных в таблицу
          ...
          { 
            ...
            $table[html] .= "<tr><td>{$row[min_datetime_log]} - {$row[max_datetime_log]}</td><td>{$protocols[$row[l4]]}</td><td>{$row[original_source_ip]}</td>
